...
if ! ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "${MASTER_USER}@${MASTER_IP}" "sudo cat /var/lib/rancher/k3s/server/node-token" > "${TOKEN_FILE}"; then
    echo "Failed to retrieve token from master."
    echo "Please verify the username ('${MASTER_USER}') and ensure passwordless SSH and sudo are configured."
    exit 1
fi
